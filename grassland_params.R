...
initialise_user_simulation_params <- function(){ 
  ...
  #simulation_params$mean_decline_rate = -0.02
  
  #simulation_params$decline_rate_std = 0.005
...
initialise_user_output_params <- function(){
  ...
  output_params$output_plot = TRUE # set to TRUE/FALSE
  ...
  output_params$scenario_vec = 'all' #'all' #c(1,4,7,10, 8, 2,3,5,6,9,11,12 ) #1:12
  ...
  output_params$site_outcome_plot_lims_set = rep(list(c(0, 3e4)), 1)
  output_params$program_outcome_plot_lims_set = rep(list(c(0e6, 1e7)), 1)
  output_params$landscape_outcome_plot_lims_set = rep(list(c(0, 2e7)), 1)
  
  output_params$site_impact_plot_lims_set = rep(list(c(-5e3, 5e3)), 1)
  output_params$program_impact_plot_lims_set = rep(list(c(-1e5, 1e5)), 1) 
  output_params$landscape_impact_plot_lims_set = rep(list(c(-5e6, 0)), 1)
